...
function MoveSet.Run(ActionName, UserInputState, InputObject)
	...
	if not Performed then return end
...
function Controller:BindAllInputs()
	for Index, Func in MoveSet do
		...
		if not KeyCode then
			warn(`InputEnums.MovementInputs.{Index} doesn't exist: See MovementInputs for more:`, InputEnums.MovementInputs)
			continue
		end
		
		if Index == 'Jump' then
			table.insert(self['Connections'], UIS.JumpRequest:Connect(function()
				Func(Index) 
			end))
			continue
		end
		...
		if KeyCode == -1 then
			if Index == "Run" then
				table.insert(self['Connections'], Mod_DoubleTap:BindDoubleTap(
					Index, 
					{'W','S','A','D'},
					.45,-- TapTimeout
					'Began',
