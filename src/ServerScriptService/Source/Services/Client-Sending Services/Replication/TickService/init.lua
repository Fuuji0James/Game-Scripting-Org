...
local BaseService = require(script.Parent._BaseService)
...
setmetatable(Service, BaseService)
...
function SetupFRS(States)	
	local ReSyncRemote = Instance.new('RemoteEvent', States.RemotesFolder)
	local RequestInfoFromServer = Instance.new('RemoteFunction', States.RemotesFolder)
	
	ReSyncRemote.Name = 'ReSyncRemote'
	RequestInfoFromServer.Name = 'RequestInfoFromServer'
end
...
function Service.new(MockTestingConfig) -- Really just an initialize function, but i call it this since its a set paradigm
	...
	local self = BaseService.new(MockTestingConfig, DefaultConfig)
	...
	States.RemotesFolder = CommsFolder -- Server-Client
	...
	SetupFRS(States) -- (Sets up Folders, Remotes & Signals)
